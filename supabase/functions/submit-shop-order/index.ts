...
serve(async (req) => {
  ...
  try {
    ...
    const { customerName, customerPhone, roomNumber, items, totalAmount, bookingIdKey, customerComment, guestId, bonusDiscount } = await req.json()
    ...
    if (guestId) {
      ...
      if (guest) {
        ...
        const originalTotal = Number(totalAmount) + Number(bonusDiscount || 0)
